...
vector<vector<double>> DataDrivenLeaderMPCCalculate(double space, double speed, double Kv, double Ku, int Np, double Ts, vector<vector<double>> sample_set){
	...
	try{
		...
		GRBVar Z[sample_set_length];
		for (int i = 0; i < sample_set_length; i++){
			ostringstream vname;
		    vname << "Z_" << i;
		    Z[i] = model.addVar(0.0, 1.0, 0.0, GRB_BINARY, vname.str());
		}
		...
		for (int i = 0; i < sample_set_length; i++){
			x_terminal += Z[i] * sample_set[i][0];
			v_terminal += Z[i] * sample_set[i][1];
            q_terminal += Z[i] * sample_set[i][2];
		}
